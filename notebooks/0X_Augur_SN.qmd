...
run_augur <- function(cell_type_column, label_column, threads, tissue, celltype, output_path) {
    ...
    if (unique(meta$broad_cell_group) == "Neuron") {
      if (group_min_cells > 200) {
        ...
        message("Subsampling neurons to {group_min_cells} cells")
      ...
      if (group_min_cells > 100) {
        ...
        message("Subsampling non-neurons to {group_min_cells} cells")
        subsample_n <- group_min_cells
      }
    }
...
batchparams_df <- 
    ...
    mutate(label_column = "microbiome") %>% 
        ...
        "{data_path}/augur/snRNASeq/{cell_type_column}/",
...
```{r}
augur_resdir <- glue("{data_path}/augur/snRNASeq")
...
saveRDS(class_aucs_df, 
  glue("{data_path}/augur/snRNASeq/class_aucs_df_{Sys.Date()}.rds")
  )
saveRDS(subclass_aucs_df, 
  glue("{data_path}/augur/snRNASeq/subclass_aucs_df_{Sys.Date()}.rds")
  )
saveRDS(supertype_aucs_df, 
  glue("{data_path}/augur/snRNASeq/supertype_aucs_df_{Sys.Date()}.rds")
  )
