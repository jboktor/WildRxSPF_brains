...
reticulate::use_condaenv("neuroestimator", required = TRUE)

## Example data
# # load example dataset.
# data(neuroestimator_example_data)
# neuroestimator_example_data$counts %>% dim()
# neuroestimator_example_data$counts[1:5, 1:5]

# # run neuroestimator.
# results <- neuroestimator(neuroestimator_example_data$counts, species = "mmusculus")
# head(results)
# results %>% glimpse()
# neuroestimator_example_data$counts %>% rownames()

#______________________________________________________________________________
# Trying with Spatial Transcriptomics data!
#______________________________________________________________________________
merged_rois <- readRDS(
    glue(
      "{wkdir}/data/interim/",
...
sk_wkdir <- glue(
  ...
  "snRNAseq_analysis/WILDRxSPF_brains/data/interim/"
  )
source(glue("{wkdir}/notebooks/R_scripts/_misc_functions.R"))

sparse_counts <- readRDS(
  glue(
    "{wkdir}/data/interim/spatial_data/",
    "dgCMatrix_singlenuclei_filtered_2025-04-28.rds"
  )
)
# sparse_counts %>% rownames() %>% head()

# ncol(sparse_counts)
# 411875

sparse_counts_chunks <- chunk_func(1:ncol(sparse_counts), 21)
sparse_counts_chunks %>% length()
sparse_counts_chunks[[1]] %>% length()

# sparse_counts[, sparse_counts_chunks[[1]]]

for (ii in 1:length(sparse_counts_chunks)) {
  message(glue("\n\nProcessing chunk {ii} of {length(sparse_counts_chunks)}"))
  counts_input <- sparse_counts[, sparse_counts_chunks[[ii]]] %>% as.matrix()
  ...
  saveRDS(results_spatial,
    glue(
      ...
      "neuroestimator_results_spatial_{ii}_{Sys.Date()}.rds"
    )
  )
}
...
#   glue(
#     "/central/groups/MazmanianLab/joeB/snRNAseq_analysis/WILDRxSPF_brains/data/interim/neuroestimator/",
#     "neuroestimator_results_spatial_2025-04-28.rds"
#   )
# )