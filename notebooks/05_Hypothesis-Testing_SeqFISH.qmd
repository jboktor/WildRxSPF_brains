...
Manual UMAP  - visualizing cell type labels and group distributions

```{r}
umap_coords <- Embeddings(merged_rois, reduction = "umap") %>%
  cbind(meta_df) %>% 
  glimpse()

p_celltype_umap <- umap_coords %>% 
  slice_sample(prop = 1) %>%  # random shuffle row order
  ggplot(aes(x = umap_1, y = umap_2, color = singleR_labels)) +
...
library(aplot)
combined_umap <- p_celltype_umap %>% insert_right(p_celltype_umap_group)
